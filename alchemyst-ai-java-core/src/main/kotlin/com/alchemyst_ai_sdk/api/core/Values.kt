...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { AlchemystAiInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw AlchemystAiInvalidDataException("`$name` is not set")
            is JsonNull -> throw AlchemystAiInvalidDataException("`$name` is null")
            else -> throw AlchemystAiInvalidDataException("`$name` is invalid, received $this")
        }
